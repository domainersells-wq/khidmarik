...
function DigitalServicesContent({ searchParams: initialSearchParamsPromise }: DigitalServicesPageProps) {
  const { translate } = useLanguage();
  ...
  useEffect(() => {
    document.title = translate('digitalServicesTitle', 'Digital Services') + ' | Khidmatik';
  }, [translate]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Digital categories are stored with type 'freelancer'
        const digCats = cats.filter(c => (c.type as string) === 'freelancer' || c.slug.includes('design') || c.slug.includes('development') || c.slug.includes('marketing') || c.slug.includes('writing'));
  ...
  // Filter freelancers client-side based on inputs
  ...
  const currentCategoryDetails = selectedCategorySlug ? digitalCategories.find(c => c.slug === selectedCategorySlug) : null;
    ...
    <div className="space-y-8">
      ...
      >
        ...
        >
          ...
          <SelectContent>
            <SelectItem value={ALL_DIGITAL_CATEGORIES_SENTINEL_VALUE}>{translate('allDigitalCategories', 'All Digital Categories')}</SelectItem>
...
export default function DigitalServicesPage(props: DigitalServicesPageProps) {
  return (
