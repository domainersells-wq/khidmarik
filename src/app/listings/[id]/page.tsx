...
import { 
  MapPin, Phone, Globe, Mail, Clock, Tag, Building, Briefcase, 
  CheckCircle, Award, ShieldCheck, Users, Cog, ShoppingBag, 
  Sparkles, Warehouse, AlertTriangle, CalendarClock, MessageSquare,
  Facebook, Instagram, Twitter, Linkedin, Heart, HelpCircle, Video, Eye,
  ArrowRightLeft
} from 'lucide-react';
...
export default function ListingDetailPage() {
  ...
  // Default Mock FAQs
  const defaultFaqs = [
    { question: "What is your average response time?", answer: "We typically respond to inquiries and chat messages within 15-30 minutes." },
    { question: "Do you offer warranty on installations?", answer: "Yes, all professional installation services include a 12-month Khidmatik-backed warranty." },
    { question: "Do you accept digital payments?", answer: "Yes, we accept CIB, Dahabia cards, and cash on service delivery." }
  ];
  ...
  useEffect(() => {
    const fetchListing = async () => {
      ...
      setListing(data);
      if (data) {
        document.title = `${data.name} | Khidmatik`;
      } else {
  ...
  const handleHelpfulVote = (reviewId: string) => {
    ...
    toast({ title: "Vote counted! / تم تسجيل إعجابك" });
  };

  const handleOpenComparison = async () => {
    // Pick another random listing from the same category to compare
    if (!listing) return;
    const related = await storeService.getStores({ categorySlug: allCategories.find(c => c.name === listing.category)?.slug });
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-16">
          ...
          <div className="w-full h-full relative overflow-hidden flex items-center justify-center">
            ...
            >
              <div className="w-[1200px] h-[350px] relative flex gap-2">
                <img src={listing.images[0] || 'https://placehold.co/800x400.png'} className="w-[500px] h-full object-cover brightness-90" />
                <img src={listing.images[1] || listing.images[0] || 'https://placehold.co/800x400.png'} className="w-[500px] h-full object-cover brightness-90" />
              </div>
            </div>
            {/* controls */}
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-black/75 p-2 rounded-full border border-white/10 z-10">
              <Button size="sm" variant="ghost" className="text-white hover:text-primary" onClick={() => setPanoramaRotation(p => p - 30)}>◀</Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-headline font-bold text-xl">{isRtl ? 'حول النشاط التجاري' : `About ${listing.name}`}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="leading-relaxed text-muted-foreground">{listing.description}</p>
              
              {/* Feature checkboxes representation */}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-headline font-bold text-xl">{isRtl ? 'معرض الصور والفيديو' : 'Media Gallery'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {listing.images.map((src, index) => (
                  <div key={index} className="aspect-video rounded-xl overflow-hidden border shadow-sm group cursor-pointer relative">
                    <img 
                      src={src} 
                      alt="gallery visual" 
                ...
                <div className="aspect-video rounded-xl border bg-slate-900 flex flex-col items-center justify-center text-center p-4 text-white relative cursor-pointer">
                  ...
                  <span className="text-[10px] font-bold uppercase">{isRtl ? 'شاهد عرض الفيديو' : 'Play Video Promo'}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {storeListing?.groupOrderItems && storeListing.groupOrderItems.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="font-headline font-bold text-xl">{isRtl ? 'صفقات شراء جماعي مميزة' : 'Group Order Deals'}</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <CardContent className="space-y-4">
              {defaultFaqs.map((faq, idx) => (
                <div key={idx} className="space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed pl-4 border-l border-primary/30">A: {faq.answer}</p>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card id="reviews">
            <CardHeader>
              <CardTitle className="font-headline font-bold text-xl">{isRtl ? 'آراء ومراجعات الزبائن' : 'Reviews & Ratings'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <ReviewList 
                reviews={listing.reviews.map(r => ({
                  ...r,
                  // Enable interactive votes for mockup presentation
                  helpfulVotes: (helpfulVotes[r.id] || 0),
                  onHelpfulClick: () => handleHelpfulVote(r.id)
                })) as any} 
              />
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <Card className="shadow-lg border-2 border-primary/20 sticky top-20 bg-card">
            <CardHeader className="bg-primary/5 pb-4">
              <div className="flex items-center justify-between">
                <CardTitle className="font-headline font-bold text-lg">{isRtl ? 'حجز موعد فوري' : 'Book Instantly'}</CardTitle>
                <Badge variant="outline" className="bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase">{isRtl ? 'مؤكد تلقائياً' : 'Auto Confirm'}</Badge>
              </div>
            </CardHeader>
            <CardContent className="p-5 space-y-4 text-xs font-semibold">
              <div className="flex items-center gap-3 border-b pb-2">
                ...
                <div>
                  <span className="text-[10px] text-muted-foreground block uppercase font-bold">{isRtl ? 'مواعيد العمل اليوم' : 'Hours Open Today'}</span>
              ...
              {listing.pricing && (
                <div className="flex items-center gap-3 border-b pb-2">
                  ...
                  <div>
                    <span className="text-[10px] text-muted-foreground block uppercase font-bold">{isRtl ? 'فئة السعر' : 'Price Level'}</span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-headline font-bold text-base">{isRtl ? 'بيانات الاتصال' : 'Business Info'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-xs font-semibold text-muted-foreground">
              {listing.location.fullAddress && (
                <div className="flex items-start gap-2.5">
                  ...
                  <span className="text-foreground">{listing.location.fullAddress}, {listing.location.city}</span>
                </div>
              )}
              ...
              {listing.contact.phone && (
                <div className="flex items-center gap-2.5">
                  ...
                  <a href={`tel:${listing.contact.phone}`} className="text-foreground hover:underline">{listing.contact.phone}</a>
                </div>
              )}
              ...
              {listing.contact.email && (
                <div className="flex items-center gap-2.5">
                  ...
                  <a href={`mailto:${listing.contact.email}`} className="text-foreground hover:underline truncate">{listing.contact.email}</a>
                </div>
              )}
              ...
              {listing.contact.website && (
                <div className="flex items-center gap-2.5">
                  ...
                  <a href={listing.contact.website} target="_blank" rel="noreferrer" className="text-foreground hover:underline truncate">
                    {listing.contact.website.replace(/^https?:\/\//, '')}
                  </a>
                </div>
              )}

              {/* Social links handles wrapper */}
              <div className="flex items-center gap-3.5 pt-3 border-t justify-center">
                <a href="#" className="p-2 bg-muted hover:bg-primary/10 hover:text-primary rounded-full transition-colors"><Facebook className="h-4 w-4" /></a>
                <a href="#" className="p-2 bg-muted hover:bg-primary/10 hover:text-primary rounded-full transition-colors"><Instagram className="h-4 w-4" /></a>
                <a href="#" className="p-2 bg-muted hover:bg-primary/10 hover:text-primary rounded-full transition-colors"><Twitter className="h-4 w-4" /></a>
                <a href="#" className="p-2 bg-muted hover:bg-primary/10 hover:text-primary rounded-full transition-colors"><Linkedin className="h-4 w-4" /></a>
              </div>
            </CardContent>
          </Card>
        </div>
