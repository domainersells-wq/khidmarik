...
const defaultAdvertisements: BannerItem[] = [
  { id: 'ad1', src: '/images/banners/ramadan_offers.png', alt: 'Ramadan Special Offers in Algérie', dataAiHint: 'ramadan promotion' },
  { id: 'ad2', src: '/images/banners/tech_products.png', alt: 'New Tech Arrivals on Khidmatik', dataAiHint: 'electronics sale' },
  { id: 'ad3', src: '/images/banners/trusted_services.png', alt: 'Discover Local Services on Khidmatik', dataAiHint: 'local services' },
  { id: 'ad4', src: '/images/banners/handmade_crafts.png', alt: 'Support Local Artisans - Handmade in Algérie', dataAiHint: 'handmade crafts algeria' },
];
  ...
  useEffect(() => {
    try {
      ...
      if (storedBannersJSON) {
        ...
        if (Array.isArray(storedBanners) && storedBanners.length > 0 && storedBanners.every(b => b.id && b.src && b.alt)) {
          setAdvertisements(storedBanners.map((b) => ({
            id: b.id,
            src: b.src,
            alt: b.alt,
            link: b.link,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const localStores = stores.filter(
          (store): store is Store => store.isMadeInAlgeria === true
        ).slice(0, 3);
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-10">
              {advertisements.map((_, index) => (
                <button
                  ...
                  className={`h-2 w-2 rounded-full transition-all duration-300 ${currentAdIndex === index ? 'bg-primary scale-125' : 'bg-muted-foreground/50 hover:bg-muted-foreground'}`}
      ...
      <section className="text-center py-8 md:py-12 animate-slide-up animation-delay-100">
        <div className="container mx-auto px-4">
          ...
          <p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-3xl mx-auto">
            {translate('welcomeSubtitle', 'Your Super-App for Algérie! Discover stores, services, parts, and more.')}
          </p>
          <div className="max-w-3xl mx-auto">
            <Suspense fallback={<div className="h-12 bg-slate-100 dark:bg-slate-900 rounded-xl animate-pulse" />}>
              <AdvancedSearchBar />
            </Suspense>
          </div>
        </div>
      ...
      <section className="animate-slide-up animation-delay-200">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {categoriesList.slice(0, 15).map((category) => {
              ...
              return (
                <Link key={category.id} href={`/listings?category=${category.slug}&type=${category.type === 'all' ? '' : category.type}`} passHref>
        ...
        <div className="text-center mt-8">
          <Button asChild variant="outline" size="lg">
            <Link href="/listings">
              <ListFilter className="mr-2 h-4 w-4" /> {translate('viewAllListings', 'View All Categories & Listings')}
            </Link>
          </Button>
        </div>
      </section>
      ...
      {!isLoading && madeInAlgeriaStores.length > 0 && (
        <section className="animate-slide-up animation-delay-500">
          <h2 className="text-3xl font-bold font-headline text-center mb-8 flex items-center justify-center">
            <Sparkles className="h-8 w-8 mr-3 text-accent" /> {translate('madeInAlgeria', 'Proudly Made in Algérie')}
          </h2>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
           ...
           <div className="text-center mt-8">
            <Button asChild variant="outline" size="lg">
              <Link href="/listings?q=made+in+algerie">
                {translate('exploreAllLocal', 'Explore All Local Products')} <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          </div>
        </section>
      ...
      <section className="py-12 bg-card rounded-lg shadow animate-slide-up animation-delay-600 border">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-lg text-muted-foreground mb-6">
            {translate('joinKhidmatikToday', 'Join Khidmatik today and reach more customers in your area.')}
          </p>
          <Button size="lg" className="bg-accent hover:bg-accent/90 text-accent-foreground" asChild>
            <Link href="/register/choice">{translate('listYourBusiness', 'List Your Business or Service')}</Link>
          </Button>
        </div>
      </section>
    </div>
