...
export function MarketplaceClient() {
  ...
  const provinceOptions = algerianWilayas.map((wilaya) => ({
    value: wilaya.code,
    label: `${wilaya.name_fr} (${wilaya.name})`,
  }));
    ...
    <div className="space-y-10">
      <header className="text-center py-10 bg-gradient-to-br from-accent/15 via-background to-primary/15 rounded-lg shadow-inner">
        ...
        <p className="text-lg md:text-xl text-foreground mt-3 max-w-3xl mx-auto">
          {translate(
            'marketplaceSub',
            'Your specialized hub for buying and selling new & used spare parts in Algérie. Find what you need, or list what you have!'
          )}
        </p>
      </header>
      ...
      <Card className="bg-accent/10 border-accent/30 shadow-lg">
        ...
        <CardContent>
          ...
          <ul className="list-disc list-inside text-sm space-y-1">
            <li>
              <strong className="text-accent">{translate('smartRequestMatching', 'Smart Request Matching')}:</strong>{' '}
              {translate(
                'smartRequestMatchingDesc',
                'When you request a part, our AI searches listings and notifies you of matches.'
              )}
            </li>
          </ul>
        </CardContent>
